...
static GstElement *pipeline, *src, *capsfilter, *rtpjitterbuffer, *rtph264depay, *tee, *decoder, *appsink, *h264parser, *muxer, *filesink, *queue_record, *queue_display;
static GstBus *bus;
...
void startRecording() {
	...
	sprintf(file_name, "/home/archana/rec%d.mp4", counter++);
	g_print("file created %s\n",file_name);
	g_object_set(filesink, "location", file_name, NULL);
	free(file_name);

	gst_bin_add_many(GST_BIN(pipeline), gst_object_ref(queue_record), gst_object_ref(h264parser), gst_object_ref(muxer), gst_object_ref(filesink), NULL);
...
int main(int argc, char *argv[])
{
	...
	if (!pipeline || !src || !capsfilter || !rtpjitterbuffer || !rtph264depay || !tee || !queue_display || !decoder || !appsink) {
		g_error("Failed to create elements");
		return -1;
	}

	gst_bin_add_many(GST_BIN(pipeline), src, capsfilter, rtpjitterbuffer, rtph264depay, tee, queue_display, decoder, appsink, NULL);
	if ( !gst_element_link_many(src, capsfilter, rtpjitterbuffer, rtph264depay, tee, NULL) 
